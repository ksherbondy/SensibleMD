...
function derivedId(file: CollectionInput): DocumentId {
  ...
  if (file.size !== undefined || file.lastModified !== undefined)
    return browserDocumentId(file);
...
export function createCollection(
  ...
  return files.map((file) => {
    ...
    return {
      id: (seen === 0 ? base : `${base}-${seen + 1}`) as DocumentId,
      name: file.name,
      source: file.source,
    };
  });
...
export function replaceCollectionDocument(
  documents: CollectionDocument[],
  id: DocumentId,
  source: string,
) {
  return documents.map((document) =>
    document.id === id ? { ...document, source } : document,
  );
}

export function adjacentCollectionDocument(
  documents: CollectionDocument[],
  activeId: DocumentId,
