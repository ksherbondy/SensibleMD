...
export function useReadingObservation({
  documentId,
  source,
  enabled,
  revision,
  ...
  useLayoutEffect(() => {
    ...
    const targets = nodes.flatMap((node) => {
      const element = surface.querySelector(
        `[id="${node.type === "heading" ? node.id : `reader-node-${node.id}`}"]`,
      );
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        const candidates = targets.filter((target) =>
          visible.has(target.element),
        );
        candidates.sort((a, b) => {
          const distance = (element: Element) => {
            const entry = visible.get(element)!;
            const top = entry.rootBounds?.top ?? 0;
            return entry.boundingClientRect.top <= top &&
              entry.boundingClientRect.bottom >= top
              ? 0
              : Math.abs(entry.boundingClientRect.top - top);
          };
          return distance(a.element) - distance(b.element);
        });
        if (candidates[0]) {
          commit.current(candidates[0].nodeId);
          isCurrent = work.capture();
        }
      },
      {
        root: null,
    ...
    const scroll = (event: Event) => {
      ...
      if (
        !armed ||
        ![document, document.documentElement, surface].includes(
          event.target as Document | Element,
        )
      )
        return;
      if (!scrolled) {
