...
export function searchCollection(
  ...
  return searchable.flatMap((document) =>
    searchDocument(parseSemanticDocument(document.source, 0), query)
      .filter((result) =>
        scope === "headings"
          ? result.nodeType === "heading"
          : scope === "links"
            ? result.nodeType === "link"
            : scope === "code"
              ? result.nodeType === "code"
              : scope === "section"
...
export function searchResultIndexAfter(
  index: number,
  resultCount: number,
