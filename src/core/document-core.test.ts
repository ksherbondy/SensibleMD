...
describe("semantic documents", () => {
  it("preserves headings and groups search results by section", () => {
    ...
    expect(searchDocument(document, "search")[0]).toMatchObject({
      section: "Search",
      occurrences: 1,
    });
  });
...
describe("pagination", () => {
  it("groups complete semantic blocks without changing their source", () => {
    ...
    expect(
      pages[1].fragments.map((fragment) => fragment.source).join("\n\n"),
    ).toContain("## Second");
  });
  ...
  it("keeps a heading with its whole oversized paragraph", () => {
    ...
    expect(
      pages
        ...
        .filter((fragment) => fragment.nodeId === document.nodes[1].id),
    ).toHaveLength(1);
    expect(
      pages[1].fragments.some((fragment) =>
        fragment.source.startsWith("## Second"),
      ),
    ).toBe(true);
  });
...
describe("structural heading commands", () => {
  it("promotes and demotes only a heading at the requested line", () => {
    const source = "# Title\n\n### Detail\n\nPlain text";
    expect(changeHeadingLevel(source, 3, "promote")).toEqual({
      from: 9,
      to: 12,
      insert: "##",
    });
    expect(changeHeadingLevel(source, 3, "demote")).toEqual({
      from: 9,
      to: 12,
      insert: "####",
    });
...
describe("accessibility diagnostics", () => {
  it("distinguishes structural errors from intent-dependent warnings", () => {
    ...
    expect(findings.find((item) => item.ruleId === "LINK-001")).toMatchObject({
      severity: "error",
      confidence: "high",
      line: 3,
    });
    expect(findings.find((item) => item.ruleId === "LINK-003")).toMatchObject({
      severity: "warning",
      confidence: "medium",
    });
  });
  ...
  it("finds empty list items and table header quality issues", () => {
    ...
    expect(findings.map((item) => item.ruleId)).toEqual(
      expect.arrayContaining(["LIST-002", "TABLE-002", "TABLE-003"]),
    );
    expect(findings.find((item) => item.ruleId === "TABLE-002")).toMatchObject({
      line: 6,
      severity: "warning",
    });
  });
...
describe("reader navigation", () => {
  it("moves only between semantic headings and resolves bookmarks by ID", () => {
    const headings = parseSemanticDocument(
      "# First\n\nText\n\n## Second\n\nText",
      1,
    ).headings;
    expect(adjacentHeading(headings, headings[0].id, "next")?.text).toBe(
      "Second",
    );
    expect(adjacentHeading(headings, headings[1].id, "next")).toBeNull();
    expect(adjacentHeading(headings, "", "previous")?.text).toBe("Second");
    expect(
      bookmarkedHeadings(headings, [headings[1].id]).map(
  ...
  it("traverses source-ordered reader node types without changing source blocks", () => {
    ...
    const paragraph = adjacentNavigableNode(
      document.navigableNodes,
      "",
      "paragraph",
      "next",
    );
    ...
    expect(
      adjacentNavigableNode(document.navigableNodes, "", "image", "next")?.text,
    ).toBe("Diagram");
    expect(
      adjacentNavigableNode(document.navigableNodes, "", "code", "next")?.text,
...
describe("command palette matching", () => {
  it("finds commands through titles and semantic keywords", () => {
    ...
    expect(
      matchCommands(commands, "forward heading").map((command) => command.id),
    ).toEqual(["reader.nextHeading"]);
    expect(
      matchCommands(commands, "save").map((command) => command.id),
    ).toEqual(["file.save"]);
  });
});
...
describe("document collections", () => {
  it("keeps each selected Markdown file as an independently addressable chapter", () => {
    const collection = createCollection([
      { name: "01 Intro.md", source: "# Intro" },
      { name: "02 Guide.md", source: "# Guide" },
    ]);
    expect(new Set(collection.map((document) => document.id)).size).toBe(2);
    expect(
      replaceCollectionDocument(collection, collection[1].id, "# Updated")[1]
        .source,
    ).toBe("# Updated");
    expect(collection[0].source).toBe("# Intro");
    expect(
      adjacentCollectionDocument(collection, collection[0].id, "next")?.name,
    ).toBe("02 Guide.md");
    expect(
      adjacentCollectionDocument(collection, collection[0].id, "previous"),
    ).toBeNull();
  });
});
...
describe("collection search", () => {
  it("preserves document and section context across collection results", () => {
    const collection = createCollection([
      { name: "Intro.md", source: "# Intro\n\nFind this." },
      { name: "Guide.md", source: "# Guide\n\nFind that." },
    ]);
    expect(
      searchCollection(collection, collection[0].id, "find", "document"),
    ).toHaveLength(1);
    expect(
      searchCollection(collection, collection[0].id, "find", "collection"),
    ).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ documentName: "Guide.md", section: "Guide" }),
      ]),
    );
    expect(
      searchCollection(collection, collection[0].id, "intro", "headings")[0],
    ).toMatchObject({ nodeType: "heading" });
  });
...
describe("reader preferences", () => {
  it("uses safe defaults and bounds layout-affecting values", () => {
    ...
    ).toMatchObject({ fontScale: 150, lineHeight: 1.3, contentWidth: 480 });
  });
});
...
describe("section summaries", () => {
  it("counts semantic content only until the next peer heading", () => {
    ...
    expect(summary).toMatchObject({
      heading: "Intro",
      counts: { heading: 2, paragraph: 2, list: 1, image: 1, link: 1 },
    });
  });
});
...
describe("internal Markdown links", () => {
  it("resolves an open collection chapter and heading anchor without accepting web URLs", () => {
    const collection = createCollection([
      { name: "intro.md", source: "# Intro" },
      { name: "guide.md", source: "# Install Guide\n\nText" },
    ]);
    expect(
    ...
    expect(
      resolveInternalMarkdownLink(collection, collection[0].id, "#intro"),
    ).toMatchObject({ documentId: collection[0].id });
  });
});
...
describe("reader copy", () => {
  it("copies a complete semantic source range rather than a rendered fragment", () => {
    ...
    expect(
      sourceForNode(
        document,
        document.navigableNodes.find((node) => node.type === "paragraph"),
      ),
    ).toBe("Paragraph with **formatting**.");
    expect(
      sourceForNode(
        document,
        document.navigableNodes.find((node) => node.type === "code"),
      ),
...
describe("semantic positions", () => {
  it("restores an exact node and recovers by fingerprint after nearby source changes", () => {
    const original = parseSemanticDocument(
      "# Title\n\nKeep this paragraph.\n\n## Next\n\nOther text.",
      1,
    );
    const paragraph = original.navigableNodes.find(
      (node) => node.type === "paragraph",
    );
    const position =
      createSemanticPosition(original, paragraph?.id ?? "") ?? undefined;
    expect(resolveSemanticPosition(original, position)).toMatchObject({
      confidence: "exact",
      strategy: "node-id",
    });
    const edited = parseSemanticDocument(
      "# Title\n\nNew paragraph first.\n\nKeep this paragraph.\n\n## Next\n\nOther text.",
      2,
    );
    expect(resolveSemanticPosition(edited, position)).toMatchObject({
      confidence: "high",
      strategy: "node-fingerprint",
      node: expect.objectContaining({ text: "Keep this paragraph." }),
    });
  });
  ...
  it("prefers the saved section for duplicate paragraphs and falls back to it after deletion", () => {
    ...
    const target = original.navigableNodes.filter(
      (node) => node.type === "paragraph",
    )[1];
    const position = createSemanticPosition(original, target.id) ?? undefined;
    const moved = parseSemanticDocument(
      "# First\n\nRepeated text.\n\n# Second\n\nOther text.\n\nRepeated text.",
      2,
    );
    expect(resolveSemanticPosition(moved, position)).toMatchObject({
      confidence: "high",
      node: expect.objectContaining({
        range: expect.objectContaining({ line: 9 }),
      }),
    });
    const deleted = parseSemanticDocument(
      "# First\n\nRepeated text.\n\n# Second\n\nOther text.",
      3,
    );
    expect(resolveSemanticPosition(deleted, position)).toMatchObject({
      strategy: "section-heading",
      node: expect.objectContaining({ text: "Second" }),
    });
  });
  ...
  it("uses a text anchor when a paragraph is lightly edited", () => {
    ...
    const position =
      createSemanticPosition(
        original,
        original.navigableNodes.find((node) => node.type === "paragraph")?.id ??
          "",
      ) ?? undefined;
    const edited = parseSemanticDocument(
      "# Setup\n\nInstall the package with npm today.",
      2,
    );
    expect(resolveSemanticPosition(edited, position)).toMatchObject({
