...
interface NavigationContext { documentId: string; source: string }
interface PendingProjection { context: NavigationContext; revision: number; cancelled: boolean; entered: boolean }
const sameContext = (a: NavigationContext, b: NavigationContext) => a.documentId === b.documentId && a.source === b.source
...
class NavigationWork {
  ...
  constructor(context: NavigationContext) { this.context = context }
  ...
  interruptCurrentContext() {
    ...
    for (const work of this.pending) if (sameContext(work.context, this.context)) work.cancelled = true
  }
  capture() {
    const context = this.context
    const revision = this.revision
    const passiveRevision = this.passiveRevision
    return () => this.alive && this.context === context && this.revision === revision && this.passiveRevision === passiveRevision
  }
  schedule(callback: () => void, context: NavigationContext) {
    const work = { context, revision: this.revision, cancelled: false, entered: sameContext(context, this.context) }
    this.pending.add(work)
    requestAnimationFrame(() => {
      this.pending.delete(work)
      if (this.alive && !work.cancelled && work.revision === this.revision && sameContext(work.context, this.context)) callback()
    })
  }
  dispose() {
    this.alive = false
...
export function useNavigationWork(documentId: string, source: string) {
  ...
  useLayoutEffect(() => { work.commit({ documentId, source }) }, [work, documentId, source])
