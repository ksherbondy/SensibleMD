import type { NavigableNodeType, SemanticHeading, SemanticNode } from './semantic-document'
...
export function adjacentNavigableNode(nodes: SemanticNode[], activeId: string, type: NavigableNodeType, direction: 'next' | 'previous') {
  ...
  if (activeIndex < 0) return direction === 'next' ? typedNodes[0] : typedNodes[typedNodes.length - 1]
