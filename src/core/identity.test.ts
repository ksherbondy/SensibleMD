...
describe("identity primitives", () => {
  it("accepts identifiers the main process can use as a filename component", () => {
    ...
    expect(
      isDocumentId(
        browserDocumentId({ name: "notes.md", size: 12, lastModified: 3 }),
      ),
    ).toBe(true);
    expect(
      isDocumentId(inMemoryDocumentId({ name: "notes.md", source: "# Notes" })),
    ).toBe(true);
  });
  ...
  it("rejects identifiers containing path separators or traversal", () => {
    ...
    expect(() => asDocumentId("../escape")).toThrow(
  ...
  it("keeps browser file identity stable while the file is unmodified", () => {
    ...
    expect(browserDocumentId(file)).not.toBe(
      browserDocumentId({ ...file, lastModified: 1_700_000_000_001 }),
    );
    expect(browserDocumentId(file)).not.toBe(
      browserDocumentId({ ...file, name: "other.md" }),
    );
  });
});
...
describe("collection identity", () => {
  ...
  it("gives same-named documents from different sources distinct identities", () => {
    const [first, second] = createCollection([
      {
        id: asDocumentId("doc-project-a-notes"),
        name: "notes.md",
        source: "# A",
      },
      {
        id: asDocumentId("doc-project-b-notes"),
        name: "notes.md",
        source: "# B",
      },
    ]);
  ...
  it("navigates a collection by identity rather than by name", () => {
    ...
    expect(
      adjacentCollectionDocument(documents, documents[0].id, "next")?.id,
    ).toBe(documents[1].id);
    expect(
      adjacentCollectionDocument(documents, documents[1].id, "previous")?.id,
    ).toBe(documents[0].id);
  });
});
