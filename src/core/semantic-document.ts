...
export interface SemanticHeading extends SemanticNode {
  type: "heading";
  level: number;
}
export type NavigableNodeType =
  | "heading"
  | "paragraph"
  | "list"
  | "table"
...
export function parseSemanticDocument(
  ...
  const nodes = tree.children.map((node, index) => ({
    ...
    text: toString(node),
    range: range(node),
    ...(node.type === "heading" ? { level: node.depth } : {}),
  }));
  ...
  const navigableNodes: SemanticNode[] = nodes.filter((node) =>
    ["heading", "paragraph", "list", "table", "code", "blockquote"].includes(
      node.type,
    ),
  );
  const inlineNodes: SemanticNode[] = [
    ...source.matchAll(/(!?)\[([^\]]*)\]\(([^)]+)\)/g),
  ].map((match, index) => {
...
export function searchDocument(
  ...
  return document.nodes.flatMap((node) => {
    ...
    const occurrences =
      node.text.toLocaleLowerCase().split(normalizedQuery).length - 1;
    return occurrences
      ? [
          {
            nodeId: node.id,
