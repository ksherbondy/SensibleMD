...
export interface OpenedReaderDocument {
  id: DocumentId;
  source: string;
  name: string;
...
export function NoDocument({
  ...
  onOpen: (file: OpenedReaderDocument) => void;
}) {
  ...
  useEffect(() => {
    let active = true;
    void window.sensibleMD
      ?.listRecentDocuments?.()
      .then((items) => {
        if (active) setRecents(items);
      })
      .catch(() => {
        if (active) setStatus("Recent files are unavailable.");
      });
    return () => {
      active = false;
    };
  ...
  const openRecent = async (index: number) => {
    ...
    try {
      ...
      setStatus("The recent file could not be opened.");
    } finally {
  ...
  const open = useCallback(async () => {
    ...
    try {
      ...
      setStatus("The Markdown file could not be opened.");
    } finally {
      pending.current = false;
    }
  }, [onOpen]);
  ...
  return (
    <div className="app-shell" data-dirty="false">
      <header className="topbar">
        <div className="brand">SensibleMD</div>
      </header>
          ...
          <nav aria-label="Recent documents">
            {recents.map((recent) => (
              <button
                key={recent.index}
                type="button"
                onClick={() => void openRecent(recent.index)}
              >
