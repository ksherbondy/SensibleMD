...
const headingFixture =
  "# Top\n\nOpening text.\n\n## Repeat\n\nText.\n\n## Repeat\n\nMore text.\n\n### **Formatted heading**\n\n#### Level Four\n\n##### Level Five\n\n###### Level Six";
const storageFor = (source: string) => ({
  "sensiblemd-document": source,
  "sensiblemd-name": "Navigation.md",
});
...
describe("N01 / DOG-011: page controls retain the user destination", () => {
  it("keeps advancing beyond a headingless page when a later heading enters view", async () => {
    ...
    try {
      ...
      expect(
        within(screen.getByRole("article", { name: "Page 2" })).queryAllByRole(
          "heading",
        ),
      ).toHaveLength(0);
      ...
      await scenario.settleNavigation();
      expect(
        scenario.visiblePageNumbers(),
        "Next from page 2 must stay on page 3, not snap back to Alpha",
      ).toEqual([3]);
      await scenario.clickNextPage();
      await scenario.settleNavigation();
  ...
  it("keeps moving backward across a headingless page and a previous heading", async () => {
    ...
    try {
      ...
      expect(scenario.visiblePageNumbers()).toEqual([4]);
      expect(
        within(screen.getByRole("article", { name: "Page 4" })).queryAllByRole(
          "heading",
        ),
      ).toHaveLength(0);
      await scenario.clickPreviousPage();
      ...
      expect(
        scenario.visiblePageNumbers(),
        "Previous from page 4 must stay on page 3, not snap back to Gamma",
      ).toEqual([3]);
    } finally {
      scenario.unmount();
    }
  ...
  for (const mode of ["Write", "Split"] as const) {
    it(`keeps ${mode} mode and moves the editor to the selected outline heading`, async () => {
      const scenario = await startScenario({
        storage: storageFor(headingFixture),
      });
      try {
        ...
        expect(
          ...
          "Outline navigation must not implicitly enter Read",
        ).toBe(mode);
        ...
        if (mode === "Split") {
          ...
          const target = within(preview).getByRole("heading", {
            name: "Level Four",
            level: 4,
          });
...
describe("N03 / DOG-012: outline targets the exact rendered heading", () => {
  const cases = [
    ...
    {
      label: "inline-formatted heading",
  ...
  for (const targetCase of cases) {
    it(`scrolls to the ${targetCase.label} in the reader`, async () => {
      ...
      try {
        ...
        const targets = within(reader).getAllByRole("heading", {
          name: targetCase.text,
          level: targetCase.level,
        });
        const target = targets[targetCase.occurrence];
        expect(target).toBeDefined();
        const priorScroll = scenario.lastScrollRequest();
        await scenario.clickOutlineHeading(
          targetCase.text,
          targetCase.occurrence,
        );
        await scenario.settleNavigation();
        ...
        expect(
          request?.element,
          "The scroll target must be this exact heading, not another duplicate or surface",
        ).toBe(target);
        expect(
          request,
          "The outline action must request a fresh reader scroll",
        ).not.toBe(priorScroll);
      } finally {
        scenario.unmount();
      }
    });
