...
describe('DOG-001B Home and OS open', () => {
  it('starts at Home with recents without restoring or opening any file', async () => {
    ...
    const scenario = await startScenario({ desktop, home: true, storage: { 'sensiblemd-document': '# Old browser document' } })
    try {
      expect(screen.getByRole('heading', { name: 'No document open' })).toBeInTheDocument()
      expect(screen.getByRole('button', { name: 'Open recent Recent.md' })).toBeInTheDocument()
      expect(desktop.sessions).toHaveLength(0)
      await scenario.user.click(screen.getByRole('button', { name: 'Open recent Recent.md' }))
  ...
  it('opens the requested file, restores its reader location, then accepts another OS file', async () => {
    const source = '# Start\n\n## Saved section'
    const desktop = new FakeDesktop().addFile('/Requested.md', source).addFile('/Next.md', '# Next')
    const documentId = desktop.documentIdFor('/Requested.md')
    const model = parseSemanticDocument(source, 0)
    desktop.documentState.set(documentId, { schemaVersion: 1, documentId, bookmarks: [], activeHeading: model.headings[1].id, position: createSemanticPosition(model, model.headings[1].id)!, fontScale: 100, lineHeight: 1.72, contentWidth: 760, reducedMotion: false })
    ...
    try {
      ...
      expect(document.querySelector('.outline-item[aria-current="location"]')).toHaveTextContent('Saved section')
      expect(scenario.lastScrollRequest()?.element).toBe(screen.getByRole('heading', { name: 'Saved section' }))
