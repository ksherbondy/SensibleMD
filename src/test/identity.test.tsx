...
const documentIdOf = () => document.querySelector('.app-shell')?.getAttribute('data-document-id') ?? ''
const sessionIdOf = () => document.querySelector('.app-shell')?.getAttribute('data-session-id') ?? ''
...
describe('document identity in the running application', () => {
  it('gives same-named files in different directories different identities', async () => {
    ...
    expect(first).toBe(desktop.documentIdFor('/Users/reader/project-a/notes.md'))
    expect(second).toBe(desktop.documentIdFor('/Users/reader/project-b/notes.md'))
  ...
  it('only ever asks for saved state belonging to the active document', async () => {
    ...
    const requested = new Set(desktop.stateRequests.map((request) => request.documentId))
    ...
    expect([...requested].every((id) => id === first || id === second || id === scenario.welcomeDocumentId)).toBe(true)
    scenario.unmount()
  })
  ...
  it('does not let one document read another document\'s recovery snapshot', async () => {
    ...
    desktop.openDialogResult = '/Users/reader/project-a/notes.md'
    await scenario.openDocument()
    await desktop.api.saveRecoverySnapshot({ documentId: documentIdOf(), version: 1, source: '# Project A unsaved' })
  ...
  it('establishes an ephemeral session for a desktop document and none for the sample', async () => {
    ...
    expect(documentIdOf()).toBe(desktop.documentIdFor('/Users/reader/guide.md'))
    scenario.unmount()
  })
  ...
  it('keeps the sample document under one fixed identity across restarts', async () => {
    ...
    const second = await startScenario({ storage: { 'sensiblemd-document': '# Edited sample', 'sensiblemd-name': 'Welcome.md' } })
