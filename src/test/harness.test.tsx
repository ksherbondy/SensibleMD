...
describe("CallScheduler", () => {
  it("resolves immediately in automatic order", async () => {
    ...
    await expect(
      scheduler.schedule("state:load", () => "loaded"),
  ...
  it("lets a test resolve a later call before an earlier one", async () => {
    ...
    const first = scheduler
      .schedule("state:load", () => "A")
      .then((value) => {
        settled.push(value);
      });
    const second = scheduler
      .schedule("state:load", () => "B")
      .then((value) => {
        settled.push(value);
      });
  ...
  it("reports which channels are waiting", () => {
    ...
    expect(scheduler.pending().map((call) => call.channel)).toEqual([
      "document:open",
      "recovery:load",
    ]);
...
describe("FakeDesktop", () => {
  it("authorizes an opened file and writes saves back to it", async () => {
    ...
    expect(opened).toMatchObject({
      name: "notes.md",
      source: "# Notes",
      documentId: desktop.documentIdFor("/Users/reader/notes.md"),
    });
    ...
    expect(desktop.writes).toEqual([
      { path: "/Users/reader/notes.md", source: "# Notes edited" },
    ]);
  });
  ...
  it("records recents and rotates recovery snapshots", async () => {
    ...
    expect((await desktop.api.loadRecoverySnapshot("doc"))?.source).toBe(
      "second",
    );
  ...
  it("delivers external change events to subscribers", async () => {
    ...
    const seen: string[] = [];
    const unsubscribe = desktop.api.onExternalDocumentChange(({ source }) => {
      seen.push(source);
    });
  ...
  it("can be made to fail a boundary call", async () => {
    ...
    await expect(desktop.api.loadDocumentState("doc")).rejects.toThrow(
