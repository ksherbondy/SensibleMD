...
const scroll = () => { const reader = document.querySelector('.document-reader')!; fireEvent.wheel(reader); fireEvent.scroll(document) }
const emit = async (current: Observer, targets: Element[]) => { await act(async () => current.emit(targets)) }
...
describe('N04 passive navigation observation', () => {
  ...
  for (const [id, mode] of [['N04-3', 'Write'], ['N04-4', 'Split']] as const) {
