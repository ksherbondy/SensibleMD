...
describe('DOG-007 transient dismissal', () => {
  it('dismisses search with Escape and reopens the same query and scope', async () => {
    ...
    try {
      ...
      const results = screen.getByRole('region', { name: 'Search results' }).textContent
      ...
      expect(screen.getByRole('region', { name: 'Search results' }).textContent).toBe(results)
  ...
  it('dismisses settings by Escape, click-away and trigger while retaining preferences', async () => {
    ...
    try {
      ...
      await s.user.click(trigger)
      expect(within(screen.getByRole('region', { name: 'Reading settings' })).getAllByRole('slider')[0]).toHaveValue('125')
      await s.user.click(screen.getByRole('textbox', { name: 'Search document' }))
      expect(screen.queryByRole('region', { name: 'Reading settings' })).not.toBeInTheDocument()
      await s.user.click(trigger)
      await s.user.click(trigger)
      expect(screen.queryByRole('region', { name: 'Reading settings' })).not.toBeInTheDocument()
    } finally { s.unmount() }
  ...
  it('dismisses palette by Escape and backdrop while retaining its query', async () => {
    const s = await startScenario()
    try {
      const trigger = screen.getByRole('button', { name: 'Show command palette' })
      await s.user.click(trigger)
      ...
      expect(trigger).toHaveFocus()
      await s.user.click(trigger)
      expect(await screen.findByRole('textbox', { name: 'Search commands' })).toHaveValue('settings')
      await s.user.click(screen.getByRole('textbox', { name: 'Search commands' }))
      const dialog = screen.getByRole('dialog', { name: 'Command palette' })
      await s.user.click(dialog.parentElement!)
      expect(dialog).not.toBeInTheDocument()
      await s.user.click(trigger)
      expect(await screen.findByRole('textbox', { name: 'Search commands' })).toHaveValue('settings')
      await s.user.click(screen.getByRole('button', { name: /Show Reading Settings/ }))
      expect(screen.queryByRole('dialog', { name: 'Command palette' })).not.toBeInTheDocument()
