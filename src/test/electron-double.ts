...
export interface RecoveryRecord { documentId: string; version: number; source: string; savedAt: string }
export interface WriteRecord { path: string; source: string }
...
export class FakeDesktop {
  ...
  private createApi(): SensibleAPI {
    ...
    listRecentDocuments: () => this.scheduler.schedule('recent:list', () => {
      if (this.failures.recents) throw new Error('Recent files are unavailable.')
      return this.recents.map((filePath, index) => ({ index, name: basename(filePath) }))
    }),

    // Positional lookup, matching the current main process. Sprint 4 replaces this
    // with an opaque capability identifier.
    openRecentDocument: (index: number) => this.scheduler.schedule('recent:open', () => {
      if (this.failures.open) throw new Error('The recent file could not be opened.')
      const filePath = this.recents[index]
      if (!filePath) return null
      try { return this.authorize(filePath) } catch { return null }
    }),

    saveOpenedDocument: (payload: { source: string }) => this.scheduler.schedule('document:save-opened', () => {
      if (this.failures.save) throw new Error('The file could not be saved.')
    ...
    saveDocumentAs: (payload: { name: string; source: string }) => this.scheduler.schedule('document:save-as', () => {
      if (this.failures.saveAs) throw new Error('The file could not be saved.')
    ...
    saveRecoverySnapshot: (payload: { documentId: string; version: number; source: string }) => this.scheduler.schedule('recovery:save', () => {
      ...
      this.recoveryLatest.set(payload.documentId, { documentId: payload.documentId, version: payload.version, source: payload.source, savedAt: new Date().toISOString() })
