...
const close = async (scenario: Awaited<ReturnType<typeof startScenario>>) => {
  await scenario.user.click(screen.getByRole('button', { name: 'Close document' }))
...
describe('DOG-001 Close Document', () => {
  it('closes to a true empty workspace, preserves reader memory, then opens another file cleanly', async () => {
    const scenario = await startScenario({ storage: { 'sensiblemd-document': '# First\n\n## Remember me' } })
    try {
      ...
      expect(screen.queryByRole('button', { name: 'Save Markdown file' })).not.toBeInTheDocument()
      ...
      await scenario.save()
      expect(scenario.desktop.diskContents('/Next.md')).toBe('# Next\n\nNew text. edited')
  ...
  it('refuses dirty close, keeps edits after failed/cancelled saving, and closes after a successful save', async () => {
    const scenario = await startScenario({ storage: { 'sensiblemd-document': '# Draft' } })
    try {
      await scenario.enterMode('Write')
      ...
      await close(scenario)
      expect(screen.getByRole('heading', { name: 'No document open' })).toBeInTheDocument()
      expect(scenario.desktop.diskContents('/Draft.md')).toBe('# Draft\n\nUnsaved writing.')
    } finally { scenario.unmount() }
  })
  ...
  it('does not close newer edits when an older in-flight save finishes', async () => {
    ...
    try {
      ...
      expect(scenario.desktop.diskContents('/Draft.md')).toBe('# Draft first edit')
      expect(screen.queryByRole('heading', { name: 'No document open' })).not.toBeInTheDocument()
    } finally { scenario.scheduler.useAutomaticOrder(); scenario.unmount() }
  })
  ...
  it('flushes outgoing reader state, preserves recovery records, and restores the same file after close', async () => {
    ...
    try {
      ...
      const recovery = { documentId: id, version: 0, source: '# Recovery copy', savedAt: '2026-01-01T00:00:00Z' }
      ...
      await scenario.openDocument()
      expect(document.querySelector('.outline-item[aria-current="location"]')).toHaveTextContent('Remember')
      expect(scenario.isDirty()).toBe(false)
      expect(scenario.desktop.documentState.get(id)?.activeHeading).toBe(saved?.activeHeading)
    } finally { scenario.unmount() }
  })
  ...
  it('does not apply a late recovery result from the closed workspace to the next file', async () => {
    ...
    const load = vi.spyOn(scenario.desktop.api, 'loadRecoverySnapshot').mockImplementationOnce(() => late.promise)
    try {
      ...
      await scenario.openDocument()
      late.resolve({ documentId: scenario.desktop.documentIdFor('/A.md'), version: 1, source: '# Old recovery', savedAt: '2026-01-01T00:00:00Z' })
      await scenario.settle()
      expect(scenario.documentName()).toBe('B.md')
      expect(screen.queryByRole('button', { name: 'Restore changes' })).not.toBeInTheDocument()
      expect(scenario.isDirty()).toBe(false)
    } finally { load.mockRestore(); scenario.unmount() }
  })
