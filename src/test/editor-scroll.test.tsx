...
describe("DOG-003 editor scroll containers", () => {
  it.each(["Write", "Split"] as const)(
    "keeps the full source and native scroller in %s with navigation requesting reveal",
    async (mode) => {
      ...
      try {
              ...
              EditorView.updateListener.of((update) => {
                for (const transaction of update.transactions)
                  if (transaction.selection)
                    scrollRequests.push(transaction.scrollIntoView);
              }),
            ),
          }),
        );
        ...
        expect(
          editor.state.doc.lineAt(editor.state.selection.main.head).text,
        ).toBe("## Final section");
  ...
  it("constrains the editor height and gives CodeMirror its own overflow container", () => {
    ...
    expect(css).toMatch(
      /\.markdown-editor \.cm-scroller\s*\{[^}]*overflow:\s*auto/,
    );
