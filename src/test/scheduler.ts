...
export class CallScheduler {
  ...
  pending(channel?: string): ScheduledCall[] {
    return this.calls
      .filter(
        (call) =>
          call.status === "pending" &&
          (channel === undefined || call.channel === channel),
      )
  ...
  releaseLatest(channel?: string): ScheduledCall {
    const matching = this.calls.filter(
      (call) =>
        call.status === "pending" &&
        (channel === undefined || call.channel === channel),
    );
    ...
    if (!call)
      throw new Error(
        `No pending call to release${channel ? ` on channel "${channel}"` : ""}`,
      );
  ...
  private locate(selector?: number | string): InternalCall {
    ...
    if (!call)
      throw new Error(
        `No pending call matching ${selector === undefined ? "any channel" : JSON.stringify(selector)}`,
      );
    return call;
  }
}
