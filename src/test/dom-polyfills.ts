  ...
  emit(targets: Element[]) {
    this.callback(
      targets.map((target) => ({
        ...
        boundingClientRect: new DOMRect(0, 0, 100, 20),
        intersectionRect: new DOMRect(0, 0, 100, 20),
...
export function installDomPolyfills() {
  ...
  if (!("getClientRects" in Range.prototype))
    defineGlobal(Range.prototype, "getClientRects", () => []);
  if (!("getBoundingClientRect" in Range.prototype))
    defineGlobal(Range.prototype, "getBoundingClientRect", () => new DOMRect());
  ...
  if (!("ResizeObserver" in globalThis))
    defineGlobal(globalThis, "ResizeObserver", NoopObserver);
  defineGlobal(
    globalThis,
  ...
  if (!("createObjectURL" in URL))
    defineGlobal(URL, "createObjectURL", () => "blob:sensiblemd-test");
  if (!("revokeObjectURL" in URL))
    defineGlobal(URL, "revokeObjectURL", () => {});
