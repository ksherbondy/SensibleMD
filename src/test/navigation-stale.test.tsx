...
function holdFrames() {
  ...
  return {
    ...
    run: async (callback: FrameRequestCallback) => {
...
describe("N05 stale navigation work", () => {
  ...
  it("user reader interaction invalidates pending explicit projection", async () => {
    ...
    try {
      ...
      const current = Observer.instances.findLast(
        (item) => item.targets.size > 0,
      )!;
      await act(async () =>
        current.emit([screen.getByRole("heading", { name: "Beta" })]),
      );
      const count = scrollRequests.length;
      await frames.run(old);
      expect(active()).toBe("Beta");
  ...
  it("permits the intended cross-document projection after its destination commits", async () => {
    const hook = renderHook(
      ({ documentId, text }) => useNavigationWork(documentId, text),
      { initialProps: { documentId: "A", text: source } },
    );
    const frames = holdFrames();
    try {
      const projected = vi.fn();
      hook.result.current.invalidate();
      hook.result.current.schedule(projected, {
        documentId: "B",
        source: "New source",
      });
  ...
  it("does not revive a request when source changes away and then returns", async () => {
    const hook = renderHook(({ text }) => useNavigationWork("A", text), {
      initialProps: { text: source },
    });
    const frames = holdFrames();
    try {
      const projected = vi.fn();
  ...
  it("an intermediate document commit cannot cancel a newer destination projection", async () => {
    const hook = renderHook(
      ({ documentId }) => useNavigationWork(documentId, source),
      { initialProps: { documentId: "A" } },
    );
    const frames = holdFrames();
    try {
      const older = vi.fn();
