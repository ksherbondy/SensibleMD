...
function mediaWidth(initial: boolean) {
  ...
  const mock = vi.spyOn(window, 'matchMedia').mockImplementation((media) => ({ media, get matches() { return narrow }, onchange: null, addEventListener: (_type: string, listener: unknown) => listeners.add(listener as () => void), removeEventListener: (_type: string, listener: unknown) => listeners.delete(listener as () => void), addListener() {}, removeListener() {}, dispatchEvent: () => true }))
  return { resize: async (next: boolean) => { await act(async () => { narrow = next; listeners.forEach((listener) => listener()) }) }, restore: () => mock.mockRestore() }
}
...
describe('DOG-008 paginated reader', () => {
  it('renders an oversized paragraph intact with reference links, emphasis and hard breaks', async () => {
    const markdown = '# Big\n\nStart **bold**  \nNext [reference][ref] ' + 'word '.repeat(300) + 'END\n\n[ref]: https://example.com'
    const scenario = await startScenario({ storage: { 'sensiblemd-document': markdown } })
    try {
      ...
      expect(within(page).getByRole('link', { name: 'reference' })).toHaveAttribute('href', 'https://example.com')
  ...
  it.each([false, true])('visits all pages forward/backward in Spread (narrow=%s)', async (narrow) => {
    const media = mediaWidth(narrow)
    const scenario = await startScenario({ storage: { 'sensiblemd-document': source } })
    try {
      ...
      while (true) {
        for (const number of scenario.visiblePageNumbers()) {
          forward.add(number)
          expect(within(screen.getByRole('article', { name: `Page ${number}` })).getByRole('heading', { name: `Section ${number}` })).toBeInTheDocument()
        }
        if (screen.getByRole('button', { name: 'Next page' }).hasAttribute('disabled')) break
        await scenario.clickNextPage()
      }
      expect([...forward]).toEqual([1, 2, 3, 4, 5])
      while (!screen.getByRole('button', { name: 'Previous page' }).hasAttribute('disabled')) await scenario.clickPreviousPage()
      expect(scenario.visiblePageNumbers()).toEqual(narrow ? [1] : [1, 2])
    } finally { scenario.unmount(); media.restore() }
  })
  ...
  it('preserves a right-page semantic target across wide/narrow Spread and Page', async () => {
    ...
    try {
      ...
      await scenario.setReadingLayout('Spread')
      expect(scenario.visiblePageNumbers()).toEqual([1, 2])
      expect(document.querySelector('.outline-item[aria-current="location"]')).toHaveTextContent('Section 2')
    } finally { scenario.unmount(); media.restore() }
  })

  it('shows an empty state rather than a blank or unreachable page for definitions only', async () => {
    const scenario = await startScenario({ storage: { 'sensiblemd-document': '[ref]: https://example.com' } })
    try {
      await scenario.setReadingLayout('Page')
  ...
  it('keeps the anchored page through three-page spread turns and presentation changes', async () => {
    const media = mediaWidth(false)
    const scenario = await startScenario({ storage: { 'sensiblemd-document': [1, 2, 3].map(section).join('\n\n') } })
    try {
      await scenario.clickOutlineHeading('Section 2')
      await scenario.setReadingLayout('Spread')
      expect(scenario.visiblePageNumbers()).toEqual([1, 2])
      ...
      await media.resize(true)
      expect(scenario.visiblePageNumbers()).toEqual([3])
      expect(document.querySelector('.outline-item[aria-current="location"]')).toHaveTextContent('Section 3')
      await media.resize(false)
      await scenario.clickPreviousPage()
      expect(scenario.visiblePageNumbers()).toEqual([1, 2])
      await scenario.clickNextPage()
  ...
  it('resolves definitions across pages and renders complete footnotes once on the final page', async () => {
    const markdown = '# First\n\nA [reference][ref] with a note[^note]. ' + 'word '.repeat(80) + '\n\n# Second\n\n' + 'word '.repeat(80) + '\n\n[ref]: https://example.com "Reference title"\n\n[^note]: Footnote **emphasis** and [reference][ref].\n\n    Another footnote paragraph.'
    const scenario = await startScenario({ storage: { 'sensiblemd-document': markdown } })
    try {
      await scenario.setReadingLayout('Page')
      expect(scenario.visiblePageLabel()).toBe('Page 1 of 2')
      const first = screen.getByRole('article', { name: 'Page 1' })
      expect(within(first).getByRole('link', { name: 'reference' })).toHaveAttribute('href', 'https://example.com')
      expect(within(first).getByRole('link', { name: 'reference' })).toHaveAttribute('title', 'Reference title')
      ...
      expect(within(notes as HTMLElement).getByRole('link', { name: 'reference' })).toHaveAttribute('href', 'https://example.com')
      ...
      expect(notes.querySelector('a[data-footnote-backref]')).toHaveAttribute('href', `#${referenceId}`)
      await scenario.setReadingLayout('Spread')
